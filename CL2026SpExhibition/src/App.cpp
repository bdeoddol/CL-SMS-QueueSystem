...
void App::receivingStream(){
    ...
    while((this->_alive == true) && (this->_connected == true) ){
        ...
        if(byte_count == -1){ 
            ...
            this->_alive = false;
            this->_connected = false;
            //check for _connected, call disconnect() within main thread
            return;
        }
        ...
        while((pos = builtJsonString.find('\n')) != string::npos){
            ...
            try{
                fc = this->_cparser.parseJsonString(extractString);
            }
            ...
            if(!fc.getFullName().empty() && !fc.getPrimaryNumber().empty()){
                ...
                Group newGroup = this->_cparser.convertFormContainerToGroup(fc);
